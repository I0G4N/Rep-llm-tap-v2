...
def create_llama_model(model_name_or_path):
    ...
    tokenizer = LlamaTokenizer.from_pretrained(
        model_name_or_path,
        fast_tokenizer=True # use fast tokenizer for better performance
    )
    if tokenizer.pad_token is None:
        # if pad token is not set, add it
        tokenizer.add_special_tokens({'pad_token': '[PAD]'})

    # set the tokenizer padding side to right, padding is done from the right side
    ...
    # resize the token embeddings to be a multiple of 8
    # this is required for some models to work with DeepSpeed ZeRO optimization
    # and to ensure compatibility with the tokenizer
